...
fig_lines = [
    ...
    rf"\node[align=center,font=\tiny] at ({dec((x1+x2)/2)},0.91) {{$({{1}}/{{9}},{{4}}/{{27}})$: symmetric $S$ fails}};",
    ...
    rf"\node[align=center,font=\tiny] at ({dec((x2+x3)/2)},1.61) {{$[{{4}}/{{27}},{{4}}/{{21}})$: coexistence, $CS_E<CS_S$}};",
    rf"\fill ({dec(x3)},1.95) circle (1.4pt);",
    rf"\node[align=center,font=\tiny] at ({dec(x3)},2.27) {{$\{{4/21\}}$: coexistence, $CS_E=CS_S$}};",
    ...
    rf"\node[align=center,font=\tiny] at ({dec((x3+x4)/2)},3.01) {{$({{4}}/{{21}},{{2}}/{{9}})$: coexistence, $CS_E>CS_S$}};",
    r"\end{tikzpicture}",
]
FIGURE.write_text("\n".join(fig_lines) + "\n", encoding="utf-8")
